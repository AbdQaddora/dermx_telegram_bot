...
bot.sendMessage(chatId, "Choose your language / اختر لغتك", {
    reply_markup: {
        inline_keyboard: [
            [{ text: "English", callback_data: 'en' }, { text: "العربية", callback_data: 'ar' }]
        ]
    }
});
});
...
if (language === 'ar' || language === 'en') {
    userState[chatId] = { language: language, currentQuestion: 0 };
...
delete userState[chatId]; // Clear user state after all questions are asked
}
}
...
if (userState[chatId] && response !== 'ar' && response !== 'en') {
    askNextQuestion(chatId);
}
}
...
const chatId = query.message.chat.id;
const user = userState[chatId];
if("/start_new_consultation" === query.data){
console.log("bbbbbbbbbbbbbb")
bot.sendMessage(chatId, 'يرجى إرسال صورة لموضع الاصابة .');
return;
}
...
// Command to start a new consultation
bot.onText(/\/start_new_consultation/, (msg) => {
const chatId = msg.chat.id;
bot.sendMessage(chatId, 'يرجى إرسال صورة لموضع الاصابة .');
});
...
// Forward the photo to your personal chat with user details
bot.sendPhoto(adminChatId, photoId, {
caption: `Consultation photo received from:
